...
namespace Sol_PuntoVenta.Presentacion
{
    public partial class Frm_Area_Despacho : Form
    {
        ...
        private void Listado_ad(string cTexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Area_Despacho.Listado_ad(cTexto); // Mediacion del contenido
                this.Formato_ad();
        ...
        {
            if (string.IsNullOrEmpty(Convert.ToString(Dgv_Listado.CurrentRow.Cells["codigo_ad"].Value))) // Convierte a string para evaluarlo
            {
                MessageBox.Show("Selecciona un registro", "Aviso del sistema", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else
            {
                this.nCodigo = Convert.ToInt32(Dgv_Listado.CurrentRow.Cells["Codigo_ad"].Value); // Convierte entero porque la variable es de tipo entero y se envia al sql si o si tipo entero
