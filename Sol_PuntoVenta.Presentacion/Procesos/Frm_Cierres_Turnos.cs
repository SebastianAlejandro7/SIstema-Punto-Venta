...
namespace Sol_PuntoVenta.Presentacion.Procesos
{
    public partial class Frm_Cierres_Turnos : Form
    {
        ...
        private void Selecciona_item()
        {
            ...
            {
                //this.nCodigo = Convert.ToInt32(Dgv_Listado.CurrentRow.Cells["Codigo_pv"].Value); // Convierte entero porque la variable es de tipo entero y se envia al sql si o si tipo entero
        ...
        private void Estado_gestion_turno_pv(int nCodigo_pv)
        {
            ...
            if (Tablax.Rows.Count > 0)
            {
                ...
                Txt_fecha_trabajo.Text = cFecha_ct.Substring(0, cFecha_ct.Length - 12);
            ...
            else
            {
                ...
                Txt_fecha_trabajo.Text = ""; // valor vacio
                this.nCodigo_tu = 0;
                Txt_turno.Text = "";
                Txt_estado.Text = "";
                this.nCodigo_pv = 0;
        ...
        private void Btn_abrirturno_Click(object sender, EventArgs e)
        {
            ...
            if (this.nCodigo_pv>0)
            {
                ...
                {
                    DialogResult Opcion;
                    Opcion = MessageBox.Show("¿Abrir el turno siguiente ahora?", "Aviso del sistema", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (Opcion == DialogResult.Yes)
                    {
                        string Rpta;
                        string cFecha_ct = Txt_fecha_trabajo.Text.Trim();
                        if (cFecha_ct == string.Empty) //asigno fecha de hoy y turno 1q en caso de que no tengamos historial de cierre del pv.
                        {
                            cFecha_ct = DateTime.Now.ToString("MM/dd/yyyy");
                            this.nCodigo_tu = 1;
                        }
                        else
                        {
                            ...
                            else if (this.nCodigo_tu == 2)
                            {
                                ...
                                cFecha_ct.Substring(0, cFecha_ct.Length - 12);
                                this.nCodigo_tu = 1;
                            }
                        }
                        Rpta = N_Cierres_Turnos.Abrir_turnos(cFecha_ct, this.nCodigo_pv, this.nCodigo_tu);
                        if (Rpta.Equals("OK"))
                        {
                            this.Estado_gestion_turno_pv(this.nCodigo_pv);
