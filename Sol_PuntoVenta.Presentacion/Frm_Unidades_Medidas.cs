...
namespace Sol_PuntoVenta.Presentacion
{
    public partial class Frm_Unidades_Medidas : Form
    {
        ...
        private void Listado_um(string cTexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Unidades_Medidas.Listado_um(cTexto); // Mediacion del contenido
                this.Formato_um();
        ...
        {
            if (string.IsNullOrEmpty(Convert.ToString(Dgv_Listado.CurrentRow.Cells["codigo_um"].Value))) // Convierte a string para evaluarlo
            {
                MessageBox.Show("Selecciona un registro", "Aviso del sistema", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else
            {
                this.nCodigo = Convert.ToInt32 (Dgv_Listado.CurrentRow.Cells["Codigo_um"].Value); // Convierte entero porque la variable es de tipo entero y se envia al sql si o si tipo entero
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string Rpta = "";
                    E_Unidades_Medidas oPrpiedad = new E_Unidades_Medidas();
                    oPrpiedad.Codigo_um = this.nCodigo;
                    oPrpiedad.Descripcion_um = Txt_descripcion.Text.Trim();
                    Rpta = N_Unidades_Medidas.Guardar_um(this.Estadoguarda, oPrpiedad);
                    if (Rpta.Equals("OK"))
                    {
