...
namespace Sol_PuntoVenta.Presentacion
{
    public partial class Frm_Marcas : Form
    {
        ...
        private void Listado_ma(string cTexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Marcas.Listado_ma(cTexto); // Mediacion del contenido
                this.Formato_ma();
        ...
        {
            if (string.IsNullOrEmpty(Convert.ToString(Dgv_Listado.CurrentRow.Cells["codigo_ma"].Value))) // Convierte a string para evaluarlo
            {
                MessageBox.Show("Selecciona un registro", "Aviso del sistema", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else
            {
                this.nCodigo = Convert.ToInt32 (Dgv_Listado.CurrentRow.Cells["Codigo_ma"].Value); // Convierte entero porque la variable es de tipo entero y se envia al sql si o si tipo entero
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string Rpta = "";
                    E_Marcas oPrpiedad = new E_Marcas();
                    oPrpiedad.Codigo_ma = this.nCodigo;
                    oPrpiedad.Descripcion_ma = Txt_descripcion.Text.Trim();
                    Rpta = N_Marcas.Guardar_ma(this.Estadoguarda, oPrpiedad);
                    if (Rpta.Equals("OK"))
                    {
